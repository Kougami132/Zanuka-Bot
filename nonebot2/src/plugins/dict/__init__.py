...
update = on_regex(r"^update$", permission=SUPERUSER)
...
async def _(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        for i in range(len(items)):
            items[i]["zh_name"] = items[i]["item_name"]
            items[i]["item_name"] = items[i]["url_name"].replace("_", " ")
        for i in range(len(rivens)):
            rivens[i]["zh_name"] = rivens[i]["item_name"]
            rivens[i]["item_name"] = rivens[i]["url_name"].replace("_", " ")
        ...
        await bot.send(event, f"Done!\nWF_Items: {old_item_num} => {new_item_num}\nWF_Rivens: {old_riven_num} => {new_riven_num}")
    except ActionFailed:
